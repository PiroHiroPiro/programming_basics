...
let koushin kakutei_eki koushin_before ekikan_list = List.map
  (
    (
      fun kakutei_eki eki -> match eki with {namae=namae; saitan_kyori=saitan_kyori; temae_list=temae_list} ->
        match kakutei_eki with {namae=kakutei_namae; saitan_kyori=kakutei_saitan_kyori; temae_list=kakutei_temae_list} ->
          let kyori = get_ekikan_kyori namae kakutei_namae ekikan_list in
            if kyori = infinity
              then eki
...
(* テスト：koushin *)
let test_13_1 = koushin eki_from_iidabashi_to_korakuen [eki_from_iidabashi_to_myogadani_2; eki_shokika_ikebukuro; eki_shokika_tokyo] global_ekikan_list = [eki_from_iidabashi_to_myogadani_1; eki_shokika_ikebukuro; eki_shokika_tokyo]
